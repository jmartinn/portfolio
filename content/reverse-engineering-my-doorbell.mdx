...
It didn't. mitmproxy worked on the first try.<Sidenote>If it had pinned, this post would end here — patching an iOS binary was not how I wanted to spend a vacation Saturday.</Sidenote>
...
<MarginNote>The Asimov quote at the bottom of this post is not decoration.</MarginNote>
...
2. `join_call` authenticates the user but doesn't authorize the room. A valid JWT is enough; it doesn't have to be the right JWT.<Sidenote>Authentication asks who you are; authorization asks whether you're allowed. The signaling layer only ever asked the first question.</Sidenote>
